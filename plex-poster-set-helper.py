...
def update_plex_labels():
    ...
    if LABEL_RATING_KEYS:
        for key, item in LABEL_RATING_KEYS.items():
            ...
            url = "{base_url}/library/sections/{section_id}/all".format(
                base_url=config["base_url"], section_id=key
            )
...
def scrape_mediux(soup):
    ...
    for script in scripts:
        if "files" in script.text:
            if "title" in script.text or "set_name" in script.text:
        ...
        if media_type == "Show":
            ...
            if data["movie_id"]:
                if data_dict["set"]["movie"]:
                    ...
                    movie_data = [movie for movie in movies if movie["id"] == movie_id][
                        0
                    ]
